...
\section*{Question 1: Education}
				...
				\begin{enumerate}
					...
					\lstinputlisting[language=R, firstline=39, lastline=49]{PS01_Kim_Sein_25358755.R}
					...
					\end{verbatim}
					 ...
					 Using a $t$-distribution with $df=24$ and $t_{0.95,24}=1.711$, the 90\% confidence interval 
					 for the average IQ in the school is:
					 \[
					 (93.96,\; 102.92).
					 \]
					...
					\lstinputlisting[language=R, firstline=63, lastline=66]{PS01_Kim_Sein_25358755.R}
					...
					\lstinputlisting[language=R, firstline=79, lastline=79]{PS01_Kim_Sein_25358755.R}	
					\begin{verbatim}
						One Sample t-test
...
\section*{Question 2: Political Economy}
	...
	\vspace{.5cm}
		...
		\lstinputlisting[language=R, firstline=90, lastline=131]{PS01_Kim_Sein_25358755.R}
	...
	\vspace{0.35cm}
	\noindent I first used a simple \textbf{pairwise scatterplot matrix} (\texttt{pairs}) because it shows every bivariate relationship among $(Y, X_1, X_2, X_3)$ at once, making it easy to screen for linear trends, direction (positive/negative), and potential outliers without relying on advanced libraries. I then added three focused scatter plots of $Y$ against each of $X_1, X_2, X_3$ to describe the substantive relationships more clearly in the text.
	
	\noindent The Pearson correlation matrix provides numerical support for the visual impressions. In particular, $\mathrm{corr}(Y, X_1)=0.532$, $\mathrm{corr}(Y, X_2)=0.448$, and $\mathrm{corr}(Y, X_3)=0.464$ (fill in from the R output). Based on these values and the scatter plots, I describe the relationships in terms of direction (positive/negative), approximate strength (weak/moderate/strong), and any visible nonlinearity or outliers.
	
	\item
	Please plot the relationship between \emph{Y} and \emph{Region}? On average, which region has the highest per capita expenditure on housing assistance?

	
	\lstinputlisting[language=R, firstline=140, lastline=149]{PS01_Kim_Sein_25358755.R}
	...
	\begin{figure}[H]\centering
		\caption{\footnotesize Boxplot of $Y$ by Region, red dots indicate region means.}
		\includegraphics[width=0.65\textwidth]{q2_Y_by_region.pdf}
	\end{figure}
	
	\noindent I chose a \textbf{boxplot} because it clearly shows the distribution of $Y$ within each region (spread, median, outliers) and allows easy comparison across groups. Adding red dots for region means makes it easy to identify which region spends the most. From the output, Region 1 (Northeast) has the highest mean per-capita expenditure on housing assistance.
	\vspace{.5cm}
	\item
	...
	\lstinputlisting[language=R, firstline=156, lastline=180]{PS01_Kim_Sein_25358755.R}
